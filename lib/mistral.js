  ...
  async generateText({ prompt, maxTokens = 1000, temperature = 0.8, useReasoning = false }) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429) {
        throw new Error('Mistral rate limit exceeded. Free mode has usage limits.');
      }
      if (response.status === 402) {
        throw new Error('Mistral usage allowance exhausted. Enable pay-as-you-go or wait for reset.');
      }
